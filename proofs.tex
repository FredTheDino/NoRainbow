...
% Testat:
% - Läst andra papers, denna, simulated annealing, inpspirationen till denna, lite runt sat problem också
% - Implementerat och testat olika versioner av algoritmen för att kontrollera korrekthet
% - Experimentellt bestämt hastighetsökning 
% - Skrivit en rad bevis, som jag sedan motbevisat
% - Skriva ännu mer bevis
% - https://www.sciencedirect.com/science/article/pii/S0166218X1500476X

% Is correct: Each eq-coloring can ALWAYS be moved one step closer a "root" node. Thus all eq-colorings are connected. Since we visit all eq-nodes we try all colorings.

% Is faster: All nodes have fewer neighbors if we only visit eq-colorings, this trims all search trees and we get a faster algorithm.
